...
\begin{frame}{Condensing the past}
  ...
  \[
    \mathcal X_+^*
      =\left\{x\in\mathcal X^*: \Pr(X_{1:|x|}=x)>0\right\}.
  \]
...
\begin{frame}{Taking stock}
  ...
  \begin{itemize}
    ...
    \item Causal states are the \textbf{best summary of the past}.
  ...
  \note{Here, ``best'' means minimal sufficient for predicting the full
...
\begin{frame}{Recurrent next-token prediction}
  \setlength{\parskip}{0pt}
  \vfill
  Consider a model with parameters $\theta$ that, given hidden state $h_n$ and
  observation $x_{n+1}$, recurrently updates its hidden state and produces the
  prediction $y_{n+1}$:

  \vspace{-3mm}
  \[
    h_{n+1}=f_\theta(h_n,x_{n+1}),
    \qquad
    y_{n+1}=g_\theta(h_n,x_{n+1}).
  \]
  ...
  \[
    y_{n+1}
      =\Pr\!\left(
        X_{n+2}\mid X_{1:n+1}=x_{1:n+1}
      \right),
      \qquad \forall\,x_{1:n+1}\in\mathcal X_+^*.
  \]
  Then the recurrent state $(h_n,x_{n+1})$ is a sufficient statistic for the
  entire future.
  ...
  h_n summarizes the observations through x_n. When x_{n+1} arrives, the same
  state--input pair produces both the updated hidden state h_{n+1} and the
  distribution used to predict X_{n+2}. The dot indicates that y_{n+1} is the
  ...
  $(h_n,x_{n+1})$, which is the predictive state in the Mealy convention and
    ...
    {\centering
      ...
      \fcolorbox{iliadAccent}{iliadAccent!6}{%
        \parbox{0.82\textwidth}{\centering
          \textbf{Exact Transformer next-token prediction}\\[1mm]
