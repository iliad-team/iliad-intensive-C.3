...
\begin{frame}{What is a hidden Markov model?}
  ...
  \begin{itemize}
    ...
    \item A \textbf{hidden}, unobserved component that evolves according to
      a \textbf{Markovian dynamic}: the next hidden state depends only on
      the current hidden state.
    \item An \textbf{observed} component that depends only on the current
      hidden state in a possibly lossy way.
  \end{itemize}
  \vfill
    ...
    \begin{aligned}
      \mathcal H
        &:=\operatorname{span}\!\left\{
          \eta^{(\emptyset)}T^{(x_1)}\cdots T^{(x_n)}
          : x_{1:n}\in\mathcal X^n,\ n\geq 0
        \right\},\\
      \mathcal F
        &:=\operatorname{span}\!\left\{
          T^{(y_1)}\cdots T^{(y_m)}\phi
          : y_{1:m}\in\mathcal X^m,\ m\geq 0
        \right\}.
    \end{aligned}
  \]
